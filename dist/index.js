...
(function () {
  ...
  var Editor = function (_EventDispatcher) {
    ...
    createClass(Editor, [{
      ...
      key: 'getText',
      value: function getText(from, to) {
        return this.getContents(from, to).filter(function (op$$1) {
      ...
      value: function getChange(producer) {
        ...
        this.updateContents = function (singleChange) {
          return change = change.compose(singleChange);
        };
        producer();
      ...
      value: function insertText(from, to, text, formats, source, selection) {
        var _normalizeArguments4 = this._normalizeArguments(from, to, text, formats, source, selection);

        var _normalizeArguments5 = slicedToArray(_normalizeArguments4, 6);

        from = _normalizeArguments5[0];
        to = _normalizeArguments5[1];
        text = _normalizeArguments5[2];
        formats = _normalizeArguments5[3];
        source = _normalizeArguments5[4];
        selection = _normalizeArguments5[5];

        if (selection == null) selection = from + text.length;
        var change = this.delta().retain(from).delete(to - from);
        var lineFormat = from === to && text.indexOf('\n') === -1 ? null : this.getLineFormat(from);
      ...
      value: function insertEmbed(from, to, embed, value, source, selection) {
        var _normalizeArguments6 = this._normalizeArguments(from, to, embed, value, source, selection);

        var _normalizeArguments7 = slicedToArray(_normalizeArguments6, 6);

        from = _normalizeArguments7[0];
        to = _normalizeArguments7[1];
        embed = _normalizeArguments7[2];
        value = _normalizeArguments7[3];
      ...
      value: function deleteText(from, to, source, selection) {
        var _normalizeArguments8 = this._normalizeArguments(from, to, source, selection);

        var _normalizeArguments9 = slicedToArray(_normalizeArguments8, 4);

        from = _normalizeArguments9[0];
        to = _normalizeArguments9[1];
        source = _normalizeArguments9[2];
        selection = _normalizeArguments9[3];
      ...
      value: function getLineFormat(from, to) {
        var _normalizeArguments10 = this._normalizeArguments(from, to);

        var _normalizeArguments11 = slicedToArray(_normalizeArguments10, 2);
      ...
      value: function formatLine(from, to, formats, source) {
        var _normalizeArguments14 = this._normalizeArguments(from, to, formats, source);

        var _normalizeArguments15 = slicedToArray(_normalizeArguments14, 4);

        from = _normalizeArguments15[0];
        to = _normalizeArguments15[1];
        formats = _normalizeArguments15[2];
        source = _normalizeArguments15[3];
      ...
      value: function removeFormat(from, to, source) {
        ...
        var _normalizeArguments22 = this._normalizeArguments(from, to, source);

        var _normalizeArguments23 = slicedToArray(_normalizeArguments22, 3);

        from = _normalizeArguments23[0];
        to = _normalizeArguments23[1];
        source = _normalizeArguments23[2];
      ...
      value: function updateContents(change) {
        ...
        var selectionEvent = shallowEqual(oldSelection, selection) ? null : { selection: selection, oldSelection: oldSelection, source: source };
  ...
  var list = {
    ...
    attr: function attr(node) {
      ...
      return indent ? { list: list, indent: indent } : { list: list };
    },
    ...
    createClass(HTMLView, [{
      ...
      value: function mount(container$$1) {
        ...
        var onInput = function onInput() {
          ...
          _this4.editor.insertText(_this4.editor.selection, text, _this4.editor.getTextFormat(from));
        };
  ...
  var keymap = {
    'Ctrl+B': function CtrlB(editor) {
      return editor.toggleTextFormat(editor.selection, { bold: true });
    },
    'Ctrl+I': function CtrlI(editor) {
      return editor.toggleTextFormat(editor.selection, { italics: true });
    },
    'Ctrl+1': function Ctrl1(editor) {
      return editor.toggleLineFormat(editor.selection, { header: 1 });
    },
    'Ctrl+2': function Ctrl2(editor) {
      return editor.toggleLineFormat(editor.selection, { header: 2 });
    },
    'Ctrl+3': function Ctrl3(editor) {
      return editor.toggleLineFormat(editor.selection, { header: 3 });
    },
    'Ctrl+4': function Ctrl4(editor) {
      return editor.toggleLineFormat(editor.selection, { header: 4 });
    },
    'Ctrl+5': function Ctrl5(editor) {
      return editor.toggleLineFormat(editor.selection, { header: 5 });
    },
    'Ctrl+6': function Ctrl6(editor) {
      return editor.toggleLineFormat(editor.selection, { header: 6 });
    },
    'Ctrl+0': function Ctrl0(editor) {
      return editor.formatLine(editor.selection, {});
    }
  };
  ...
  function history(view) {
    ...
    function action(event, source, dest) {
      ...
      lastRecorded = 0;
      ignoreChange = true;
      editor.updateContents(entry[source], SOURCE_USER$3, entry[source + 'Selection']);
      ignoreChange = false;
    }
    ...
    editor.on('editor-change', function (_ref) {
  ...
  var editor$1 = new Editor();
  var view$1 = new HTMLView(editor$1, { modules: defaultViewModules });
  ...
  view$1.on('decorate', function (editor) {
