...
(function () {
  ...
  var cursor = {
    ...
    dom: function dom(node) {
      return { name: node.dataset.name, color: node.style.backgroundColor };
    },
    vdom: function vdom(value) {
    ...
    createClass(View, [{
      ...
      value: function getSelection$$1() {
        ...
        if (range && this.reverseDecorations.ops.length) {
          range = range.map(function (i) {
            return _this4.reverseDecorations.transform(i);
          });
        }
        return range;
      }
      ...
      value: function setSelection$$1(range) {
        ...
        if (range && this.decorations.ops.length) {
          range = range.map(function (i) {
            return _this5.decorations.transform(i);
          });
        }
      ...
      value: function mount(container) {
        ...
        var onKeyDown = function onKeyDown(event) {
          ...
          _this6.fire('shortcut:' + shortcut, event, shortcut);
          _this6.fire('shortcut', event, shortcut);
        ...
        var observer = new MutationObserver(function (list) {
          ...
          if (isTextChange) {
            var change = _this6.editor.delta();
            var index$$1 = getNodeIndex(_this6, mutation.target);
            index$$1 = _this6.reverseDecorations.transform(index$$1);
            ...
            var contents = deltaFromDom(_this6, _this6.root);
            contents = contents.compose(_this6.reverseDecorations);
            var _change = _this6.editor.contents.diff(contents);
        ...
        this.resumeObserver = function () {
          return observer.observe(_this6.root, opts);
        };
        ...
        var devObserver = new MutationObserver(function (list) {
          ...
          checking = setTimeout(function () {
            checking = 0;
            var diff = editor.contents.compose(_this6.decorations).diff(deltaFromDom(view));
            if (diff.length()) {
        ...
        this.unmount = function () {
          ...
          observer.disconnect();
          _this6.root.removeEventListener('keydown', onKeyDown);
          _this6.root.ownerDocument.removeEventListener('selectionchange', onSelectionChange);
          _this6.root.remove();
          _this6.unmount = function () {};
        };
      }
    }, {
      key: 'unmount',
      ...
      value: function _preventIncorrectFormats(_ref3) {
        ...
        return !change.ops.some(function (op) {
          if (_typeof(op.insert) === 'object') {
            return !_this7.paper.embeds.find(op.insert);
          } else if (op.attributes) {
            return !(_this7.paper.blocks.find(op.attributes) || _this7.paper.markups.find(op.attributes));
          }
        });
      }
    }]);
  ...
  var view$1 = new View(editor$1, { modules: defaultViewModules.concat([smartQuotes, smartEntry()]) });
  ...
  //   }
  // });
  view$1.on('update', function () {
    searchDisplay.innerHTML = '';
    if (!searchString) return;
    var text = editor$1.getText().toLowerCase();
    var lastIndex = 0,
        index = void 0;
    while ((index = text.indexOf(searchString, lastIndex)) !== -1) {
      lastIndex = index + searchString.length;
      var rects = view$1.getAllBounds(index, lastIndex);
  ...
  view$1.paper.embeds.add(cursor);
  var pos = 10;
  var changes = editor$1.delta();

  view$1.on('decorate', function (editor, event) {
