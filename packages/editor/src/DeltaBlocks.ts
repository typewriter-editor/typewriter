...
export function fromDelta(ops: Op[]): Block[] {
  const blocks: Block[] = [];
  let contents: Op[] | undefined;
...
function getBlocks(op: Op, contents?: Op[]): Block[] {
  const blocks: Block[] = [];
...
function getBlock(attributes?: Attributes, contents: Op[] = []): Block {
  if (attributes) return { attributes, contents };
  else return { contents }
}

function getContent(insert: string | object, attributes?: Attributes): Op {
